...
#define CHECK_HEX_BOUNDARIES(p, c)                                            \
	{                                                                         \
		...
		index = map.local_to_map(                                             \
				(p) + Vector3(Math_SQRT3 / 2.0 - 0.001, 0, 0.499));           \
		CHECK_MESSAGE(                                                        \
				index == (c), #p " northeast vertex not in expected cell");   \
		index = map.local_to_map((p) + Vector3(0, 0, 0.999));                 \
		CHECK_MESSAGE(index == (c), #p " north vertex not in expected cell"); \
		index = map.local_to_map(                                             \
				(p) + Vector3(-Math_SQRT3 / 2.0 + 0.001, 0, 0.499));          \
		CHECK_MESSAGE(                                                        \
				index == (c), #p " northwest vertex not in expected cell");   \
		index = map.local_to_map((p) + Vector3(0, 0, -0.499));                \
		CHECK_MESSAGE(index == (c), #p " west edge not in expected cell");    \
		index = map.local_to_map(                                             \
				(p) + Vector3(-Math_SQRT3 / 2.0 + 0.001, 0, -0.499));         \
		CHECK_MESSAGE(                                                        \
				index == (c), #p " southwest vertex not in expected cell");   \
		index = map.local_to_map((p) + Vector3(0, 0, -0.99));                 \
		CHECK_MESSAGE(index == (c), #p " south vertex not in expected cell"); \
		index = map.local_to_map(                                             \
				(p) + Vector3(Math_SQRT3 / 2.0 - 0.001, 0, -0.49));           \
		CHECK_MESSAGE(                                                        \
				index == (c), #p " southeast vertex not in expected cell");   \
	}
...
TEST_CASE("[SceneTree][GridMap] local_to_map() for hex cells") {
	...
	// southwest one cell
	CHECK_HEX_BOUNDARIES(
			Vector3(-Math_SQRT3 / 2, 0, -1.5), Vector3i(0, 0, -1));
}
...
TEST_CASE("[SceneTree][GridMap] map_to_local() for hex cells") {
	...
	CHECK(map.map_to_local(Vector3i(-1, 0, 0)) ==
			Vector3(-Math_SQRT3, 0.5, 0));
	CHECK(map.map_to_local(Vector3i(0, 0, 1)) ==
			Vector3(Math_SQRT3 / 2, 0.5, 1.5));
	CHECK(map.map_to_local(Vector3i(-1, 0, 1)) ==
			Vector3(-Math_SQRT3 / 2, 0.5, 1.5));
	CHECK(map.map_to_local(Vector3i(-1, 0, 1)) ==
			Vector3(-Math_SQRT3 / 2, 0.5, 1.5));
	CHECK(map.map_to_local(Vector3i(0, 0, -1)) ==
...
TEST_CASE("[SceneTree][GridMap] map_to_local() with square cells") {
	...
	TypedArray<Vector3i> cells =
			map.local_region_to_map(Vector3(0, 0, 0), Vector3(0, 0, 0));
	...
	cells = map.local_region_to_map(
			Vector3(-10, -100, -1000), Vector3(-11, -102, -1003));
	...
	SUBCASE("2x2x2 cube tiles") {
		...
		cells = test_map.local_region_to_map(
				Vector3(0, 0, 0), Vector3(3.9, 0, 3.9));
		CHECK(cells.has(Vector3i(0, 0, 0)));
		CHECK(cells.has(Vector3i(0, 0, 1)));
		CHECK(cells.has(Vector3i(1, 0, 0)));
...
TEST_CASE("[SceneTree][GridMap] map_to_local() with hex cells") {
	...
	// three cells along the x-axis
	cells = map.local_region_to_map(
			Vector3(0, 0, 0), Vector3(Math_SQRT3 * 2, 0, 0));
	CHECK(cells.has(Vector3i(0, 0, 0)));
	CHECK(cells.has(Vector3i(1, 0, 0)));
	CHECK(cells.has(Vector3i(2, 0, 0)));
	CHECK(cells.size() == 3);
	...
	cells = map.local_region_to_map(
			Vector3(-Math_SQRT3, 0, -1), Vector3(Math_SQRT3, 0, 1));
	CHECK(cells.has(Vector3i(0, 0, 0)));
	CHECK(cells.has(Vector3i(0, 0, 1)));
	CHECK(cells.has(Vector3i(0, 0, -1)));
	CHECK(cells.has(Vector3i(1, 0, -1)));
	...
	// cells
	cells = map.local_region_to_map(
			Vector3(-SQRT3_2 + 0.1, 0, -1), Vector3(SQRT3_2 - 0.1, 0, 1));
	CHECK(cells.has(Vector3i(0, 0, 0)));
	CHECK(cells.has(Vector3i(0, 0, 1)));
	CHECK(cells.has(Vector3i(0, 0, -1)));
	CHECK(cells.has(Vector3i(1, 0, -1)));
